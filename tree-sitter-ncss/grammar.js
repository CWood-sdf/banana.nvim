/**
 * @file NCSS grammar for tree-sitter
...
module.exports = grammar({
    ...
    rules: {
        ...
        keyframe_block: ($) =>
            seq(choice($.from, $.to, $.integer_value), $.block),
        ...
        postcss_statement: ($) =>
            prec(-1, seq($.at_keyword, repeat($._value), ";")),

        at_rule: ($) =>
            seq($.at_keyword, sep(",", $._query), choice(";", $.block)),
        ...
        attribute_selector: ($) =>
            seq(
                ...
                "[",
                alias(
                    choice($.identifier, $.namespace_selector),
                    $.attribute_name,
                ),
                optional(
                    seq(choice("=", "~=", "^=", "|=", "*=", "$="), $._value),
