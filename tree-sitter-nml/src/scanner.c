...
static void deserialize(Scanner* scanner, const char* buffer, unsigned length) {
    ...
    if (length > 0) {
        ...
        memcpy(
            &serialized_tag_count, &buffer[size], sizeof(serialized_tag_count)
        );
        ...
        if (tag_count > 0) {
            ...
            for (iter = 0; iter < serialized_tag_count; iter++) {
                ...
                if (tag.type == CUSTOM) {
                    ...
                    memcpy(
                        tag.custom_tag_name.contents, &buffer[size], name_length
                    );
                    size += name_length;
                }
...
bool tree_sitter_html_external_scanner_scan(
...
unsigned
tree_sitter_html_external_scanner_serialize(void* payload, char* buffer) {
    Scanner* scanner = (Scanner*)payload;
    return serialize(scanner, buffer);
}

void tree_sitter_html_external_scanner_deserialize(
